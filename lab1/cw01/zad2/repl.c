...
regex_t regexSHOW; // regex for the show command, id=2
...
void enableRegexes() {
    ...
    regcomp(&regexSHOW, "show [0-9][0-9]*", REG_EXTENDED); // show regex
...
bool getCommand(char* command, size_t length) {
    ...
    if (regexec(&regexSHOW, command, 0, NULL, 0) == 0) {
        cmdID = 2;
        sscanf(command, "show %d", &cmdIndex);
        return true;
    }
...
void executeCommand() {
    switch (cmdID) {
        ...
        case 2:
            ...
            getFromStructure(structurePointer, cmdIndex);
            break;
        case 3:
            if(!structureInitialised) {
                printf("Structure not initialised!\n");
